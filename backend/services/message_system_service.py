...
class MessageSystemService:
    ...
    async def send(self, sender_id: int, receiver_id: int, title: str, content: str) -> dict:
        ...
        # 3. 检查接收者是否存在
        if not MessageSystemRepository.check_user_active(receiver_id):
            return {"status_code": 404, "content": {"success": False, "message": "接收者不存在或已被禁用", "error": "RECEIVER_NOT_FOUND"}}

        # 4. 创建消息
        message_id = MessageSystemRepository.create(sender_id, receiver_id, title, content)
    ...
    async def mark_read(self, message_id: int, user_id: int) -> dict:
        ...
        # 3. 标记已读
        MessageSystemRepository.mark_read(message_id, datetime.now().isoformat())
